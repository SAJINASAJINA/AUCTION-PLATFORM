...
const SideDrawer = () => {
  ...
  return (
    <>
      ...
      <div
        className={`w-[100%] sm:w-[300px] bg-[#f6f4f0] h-full fixed top-0 z-50 ${
          show ? "left-0" : "left-[-100%]"
        } transition-all duration-100 p-4 flex-col justify-between lg:left-0 border-r-[1px] border-r-stone-500`}
      >
        <div className="relative">
          ...
          {!isAuthenticated ? (
            <>
              <div className="my-4 flex gap-2">
                ...
                <Link
                  to={"/login"}
                  className="text-[#DECCBE] bg-transparent border-[#DECCBE] border-2 hover:bg-[#fffefd] hover:text-[#fdba88] font-bold text-xl py-1 px-4 rounded-md text-white"
                >
                  Login
                </Link>
              </div>
            ...
            <>
              <div className="my-4 flex gap-4 w-fit" onClick={handleLogout}>
                <button className="bg-[#D6482B] font-semibold hover:bg-[b8381e] text-xl py-1 px-4 rounded-md">
                  Logout
                </button>
              </div>
            </>
          ...
          <IoMdCloseCircleOutline
            onClick={() => setShow(!show)}
            className="absolute top-0 righy-4 text-[28px] sm:hidden"
          />
        </div>

        <div>
          <div className="flex gap-2 item-center mb-2">
            <Link
              to="/"
          ...
          <p className="text-stone-500 font-semibold hover:text-[#D6482b] hover:transition-all hover:duration-150">
            CodeWithSAJI{" "}
          </p>
        </div>
      </div>
